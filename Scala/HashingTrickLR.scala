...
/**
 * ??????HashingTrick?LogisticRegression????CTR???????????
...
object HashingTrickLR {
  ...
  def main(args: Array[String]) {
    ...
    val csv = args(0)  // kaggle????????????????


    /** ---------------------------------------------------------------------------------------------------------*/
    // CSV(?????)?????
    val rawCsv = sqlContext.read.format( "com.databricks.spark.csv" ).option( "header", "false" ).load( "H:/Ali/Bit Briefcase/Big Data/Kaggle/CTR/ctr_set1.csv" )
    ...
    val data = rawCsv.select(
      //cast is like .as in R...it casts the column to a different data type.
      $"id",
