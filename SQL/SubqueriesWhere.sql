...
select sName
from Student
where sID in (select sID from Apply where major = 'CS');  /*real old schol way to write it */
...
where exists (select * from Student S2
              where S2.sizeHS < S1.sizeHS); /*select student where there exists some other 
student whose high school is smaller then that returned */
